...
class SnykApi {
    ...
    static async getJiraLinkedSnykIssueIds() {
        ...
        try {
            const response = await axios.get(jiraIssuesUrl, {
                headers: {
                    'Authorization': `token ${SNYK_TOKEN}`
                }
            });
    ...
    static async createJiraTicketForSnykIssue(snykIssueId, jiraProjectId, jiraSummary, jiraDesc) {
        ...
        // --- Make API Call ---
        try {
            const response = await axios.post(apiUrl, requestBody, {headers: {'Authorization': `token ${SNYK_TOKEN}`}});
...
async function populateSourceFilePath(issueSummary) {
    ...
    issueSummary.filePath = additionalAttrs.primaryFilePath;
}
